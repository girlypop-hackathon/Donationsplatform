...
function normalizeImageFileName (imageValue) {
  const normalizedValue = String(imageValue || '').trim().replace(/\\/g, '/')
...
export function resolveCampaignImageSource (campaignImageValue, fallbackFileName = 'fundtogether-logo.png') {
  ...
  const normalizedFileName = SEEDED_CAMPAIGN_IMAGE_MAP[normalizedInput] || normalizedInput
...
export function resolveLogoImageSource (logoFileName, fallbackFileName = 'fundtogether-logo.png') {
  ...
  const normalizedLogoFileName = normalizeImageFileName(rawLogoValue) || fallbackFileName
  return buildPublicImagePath(normalizedLogoFileName)
}

// Applies a fallback image source once to avoid repeated onError loops.
export function applyImageFallbackOnce (event, fallbackSource, backupFallbackSource = createImagePlaceholderDataUri()) {
  if (!event?.currentTarget || !fallbackSource) {
    return
  }

  const imageElement = event.currentTarget
  const alreadyTriedPrimaryFallback = imageElement.dataset.fallbackTried === 'true'
