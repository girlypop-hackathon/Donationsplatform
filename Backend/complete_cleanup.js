...
function startCompleteCleanup(db) {
  db.serialize(() => {
    ...
    db.all('SELECT DISTINCT organization_id FROM campaigns', [], (err, orgs) => {
      if (err) {
      ...
      orgs.forEach(org => {
        ...
        db.all(`SELECT campaign_bio, MIN(campaign_id) as keep_id 
                FROM campaigns 
                WHERE organization_id = ? 
                GROUP BY campaign_bio 
                HAVING COUNT(*) > 1`, [org.organization_id], (err, duplicates) => {
          
          if (err) {
            console.error(`Error finding duplicate campaigns for org ${org.organization_id}:`, err.message);
            return;
          }
          ...
          duplicates.forEach(dup => {
            ...
            db.run(`DELETE FROM campaigns 
                    ...
                    AND campaign_bio = ? 
                    AND organization_id = ?`,
                    [dup.keep_id, dup.campaign_bio, org.organization_id], (err) => {
                      if (err) console.error(`Error removing duplicate campaign:`, err.message);
                    });
          });
        });
      });
    ...
    db.all(`SELECT user_name, campaign_id, amount, MIN(donation_id) as keep_id 
            FROM donations 
      ...
      duplicates.forEach(dup => {
        ...
        db.run(`DELETE FROM donations 
                ...
                AND campaign_id = ? 
                AND amount = ?`,
                [dup.keep_id, dup.user_name, dup.campaign_id, dup.amount], (err) => {
    ...
    db.run(`DELETE FROM donations WHERE campaign_id NOT IN (SELECT campaign_id FROM campaigns)`, [], (err) => {
      if (err) console.error('Error cleaning up orphaned donations:', err.message);
      else console.log('✅ Cleaned up orphaned donations');
    });
    
    db.run(`DELETE FROM campaigns WHERE organization_id NOT IN (SELECT organization_id FROM organizations)`, [], (err) => {
      if (err) console.error('Error cleaning up orphaned campaigns:', err.message);
    ...
    setTimeout(() => {
      ...
      db.all('SELECT * FROM organizations ORDER BY organization_id', [], (err, rows) => {
        console.log('Organizations:');
        rows.forEach(row => console.log(`  ${row.organization_id}: ${row.name}`));
      });
      
      db.all('SELECT * FROM campaigns ORDER BY campaign_id', [], (err, rows) => {
        console.log('Campaigns:');
        rows.forEach(row => console.log(`  ${row.campaign_id}: ${row.campaign_bio} (Org: ${row.organization_id})`));
      });
      
      db.all('SELECT * FROM donations ORDER BY donation_id', [], (err, rows) => {
        console.log('Donations:');
        rows.forEach(row => console.log(`  ${row.donation_id}: ${row.user_name} -> Campaign ${row.campaign_id} (${row.amount})`));
      });
