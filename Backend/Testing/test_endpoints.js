...
async function testEndpoints() {
  ...
  try {
    ...
    console.log('✓ Donation created with ID:', donationResponse.data.data.donationId);
    ...
    const newsletterResponse = await axios.post(`${baseUrl}/newsletters/send`, {
      ...
      newsletterBody: 'This is a test newsletter message from the API test script.'
    });
    console.log('✓ Newsletter recipients:', newsletterResponse.data.data.recipientsCount);
    ...
    console.log('✓ Campaign close endpoint response:', closeResponse.data.success ? 'ok' : 'failed');
