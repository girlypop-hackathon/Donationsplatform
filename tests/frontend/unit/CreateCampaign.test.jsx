...
describe('CreateCampaign Component - TDD Tests', () => {
  ...
  test('should render create campaign form with all required fields', () => {
    ...
    expect(screen.getByLabelText('Organization/Provider *')).toBeInTheDocument();
  ...
  test('should allow editing of milestone values', () => {
    ...
    // Change the value
    fireEvent.change(milestone1, { target: { value: 'Custom milestone text' } });
  ...
  test('should show error when submitting with missing required fields', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Provider, campaign bio, and goal amount are required')).toBeInTheDocument();
    });
  });
  ...
  test('should show error when goal amount is not a positive number', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Goal amount must be a positive number')).toBeInTheDocument();
    });
  });
  ...
  test('should attempt to submit form with valid data', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('/api/campaigns'), expect.objectContaining({
      method: 'POST',
